...
(function () {
  ...
  /* ---- الإعلانات (جاهزة للتفعيل بعد قبول AdSense) ----
     بعد قبول AdSense: (1) ضع معرّف الناشر في ADSENSE_CLIENT (ca-pub-XXXXXXXXXXXXXXXX)
  ...
  function adSlot(name) {
    if (!adsEnabled() || !AD_SLOTS[name]) return "";
    return '<div class="ad-wrap"><ins class="adsbygoogle" style="display:block" data-ad-client="' +
      ADSENSE_CLIENT + '" data-ad-slot="' + AD_SLOTS[name] + '" data-ad-format="auto" data-full-width-responsive="true"></ins></div>';
  }
  function pushAds() {
    if (!adsEnabled()) return;
    try {
      Array.prototype.forEach.call(document.querySelectorAll('ins.adsbygoogle:not([data-adsbygoogle-status])'), function () {
        (window.adsbygoogle = window.adsbygoogle || []).push({});
      });
    } catch (e) {}
  }
  function initConsent() {
    if (!adsLive() || localStorage.getItem("ks_consent") || document.querySelector(".consent-bar")) return;
    var isAr = lang === "ar";
    var bar = document.createElement("div");
    bar.className = "consent-bar";
    bar.innerHTML =
      '<span>' + (isAr ? "نستخدم ملفات تعريف الارتباط لعرض الإعلانات وتحسين تجربتك. " : "We use cookies for ads and to improve your experience. ") +
      '<a href="privacy.html">' + (isAr ? "سياسة الخصوصية" : "Privacy Policy") + '</a></span>' +
      '<div class="consent-btns"><button id="cAccept">' + (isAr ? "موافق" : "Accept") + '</button>' +
      '<button id="cReject" class="ghost">' + (isAr ? "رفض" : "Reject") + '</button></div>';
    document.body.appendChild(bar);
    $("cAccept").onclick = function () { localStorage.setItem("ks_consent", "1"); bar.remove(); };
    $("cReject").onclick = function () { localStorage.setItem("ks_consent", "0"); (window.adsbygoogle = window.adsbygoogle || []).requestNonPersonalizedAds = 1; bar.remove(); };
  }
  ...
  function renderHome() {
    ...
    $("homeContent").innerHTML = html + adSlot("home_bottom");
    var privTxt = lang === "ar" ? "سياسة الخصوصية" : lang === "zh" ? "隐私政策" : lang === "ko" ? "개인정보처리방침" : lang === "es" ? "Política de privacidad" : "Privacy Policy";
    $("footer").innerHTML = L.footer + "<br><small>" + L.disclaimer + "</small>" +
      '<br><a href="privacy.html" style="color:var(--muted);font-size:11px;text-decoration:underline;display:inline-block;margin-top:8px">' + privTxt + "</a>";
    pushAds(); initConsent();
  ...
  function calcCastle() {
    ...
    function timeStepperHtml(prefix, h, m, s) {
      var isAr = lang === "ar";
      var u = [
        { k:"H", v:h, mx:23, lbl: isAr?"ساعة":lang==="zh"?"时":lang==="ko"?"시":lang==="es"?"H":"H" },
        { k:"M", v:m, mx:59, lbl: isAr?"دقيقة":lang==="zh"?"分":lang==="ko"?"분":lang==="es"?"Min":"Min" },
        { k:"S", v:s, mx:59, lbl: isAr?"ثانية":lang==="zh"?"秒":lang==="ko"?"초":lang==="es"?"Seg":"Sec" }
      ];
      function col(x) {
        return '<div class="ts-col">' +
          '<button type="button" class="ts-btn" id="' + prefix + x.k + 'up">+</button>' +
          '<input type="number" class="ts-in" id="' + prefix + x.k + '" value="' + x.v + '" min="0" max="' + x.mx + '" inputmode="numeric">' +
          '<button type="button" class="ts-btn" id="' + prefix + x.k + 'dn">−</button>' +
          '<div class="ts-lbl">' + x.lbl + '</div></div>';
      }
      return '<div class="time-stepper">' + col(u[0]) + '<span class="ts-sep">:</span>' + col(u[1]) + '<span class="ts-sep">:</span>' + col(u[2]) + '</div>';
    }
    function bindStepper(prefix, onChange) {
      ["H","M","S"].forEach(function(k) {
        var mx = k === "H" ? 23 : 59;
        var inEl = $(prefix + k), up = $(prefix + k + "up"), dn = $(prefix + k + "dn");
        if (!inEl) return;
        up.addEventListener("click", function() { var v = (parseInt(inEl.value,10)||0) + 1; inEl.value = v > mx ? 0 : v; onChange(); });
        dn.addEventListener("click", function() { var v = (parseInt(inEl.value,10)||0) - 1; inEl.value = v < 0 ? mx : v; onChange(); });
        inEl.addEventListener("input", onChange);
        inEl.addEventListener("change", function() { var v = parseInt(inEl.value,10); if (isNaN(v)) v = 0; inEl.value = ((v % (mx+1)) + (mx+1)) % (mx+1); onChange(); });
      });
    }
    function readStepper(prefix) {
    ...
    function buildOffense() {
      ...
      inp.innerHTML = modeToggleHtml() +
        ...
        '<div class="field"><label>' + hitLbl + '</label>' + timeStepperHtml("cHit", cu.h, cu.m, cu.s) + '</div>' +
    ...
    function buildDefense() {
      ...
      inp.innerHTML = modeToggleHtml() +
        ...
        '<div class="field"><label>' + dHitLbl + '</label>' + timeStepperHtml("dHit", du.h, du.m, du.s) + '</div>' +
  ...
  function calcSpecialists() {
    ...
    var specIdx = 0;
    // سُلّم «حالة العلاقة» للمتخصص (مؤكّد من اللعبة): معرفة=Acquaintance، عابرة=Casual
    var REL_STAGES = [
      { ar:"غريب", en:"Stranger" },           // مستوى 0-9
      { ar:"معرفة 1", en:"Acquaintance 1" },  // 10
      { ar:"معرفة 2", en:"Acquaintance 2" },  // 20
      { ar:"معرفة 3", en:"Acquaintance 3" },  // 30
      { ar:"عابرة 1", en:"Casual 1" },        // 40
      { ar:"عابرة 2", en:"Casual 2" },        // 50
      { ar:"عابرة 3", en:"Casual 3" },        // 60
      { ar:"قريب 1", en:"Close 1" },          // 70
      { ar:"قريب 2", en:"Close 2" },          // 80
      { ar:"قريب 3", en:"Close 3" },          // 90
      { ar:"روح قريبة", en:"Kindred Soul" }   // 100
    ];
    function stageOf(level) { return REL_STAGES[Math.min(Math.floor(level / 10), 10)]; }
    function stageName(level) { var s = stageOf(level); return lang === "ar" ? s.ar : s.en; }
    ...
    function build() {
      ...
      if (s.skills) {
        s.skills.forEach(function(sk, si) {
          ...
          skHtml +=
            ...
            '<div style="font-weight:800;font-size:14px">' + skName + "</div>" +
            '<span id="sk' + si + 'Lock" style="font-size:10px;font-weight:800;white-space:nowrap;padding:3px 9px;border-radius:999px"></span>' +
            "</div>" +
      ...
      inp.innerHTML =
        ...
        (isAr
          ? "🔒 المهارات تُفتح حسب «حالة العلاقة» مع المتخصص: المهارة 1 عند معرفة 1 (مستوى 10)، الثانية معرفة 2 (20)، الثالثة معرفة 3 (30)، الرابعة عابرة 1 (40). الشارة فوق كل مهارة تبيّن حالة العلاقة المطلوبة لفتحها. ملاحظة: رفع مستوى المهارة أكثر يتطلب حالة علاقة أعلى (اللعبة تعرض المطلوب بدقة لكل مستوى). تستهلك المهارات «مخطوطات المتخصص»."
          : "🔒 Skills unlock by the specialist's «relationship status»: skill 1 at Acquaintance 1 (Lv 10), skill 2 at Acquaintance 2 (20), skill 3 at Acquaintance 3 (30), skill 4 at Casual 1 (40). The badge shows the relationship status needed to unlock each skill. Note: raising a skill further needs a higher relationship status (the game shows the exact requirement per level). Skills consume Master's Manuscripts.") +
        "</p>" + skHtml + "</div>" +
    ...
    function compute() {
      ...
      if (s.skills) {
        s.skills.forEach(function(sk, si) {
          ...
          if (!sfEl || !stEl) return;
          // بوابة الفتح: المهارة تُفتح عند مستوى المتخصص (الترتيب × 10) = حالة علاقة معيّنة
          var unlockLv = (si + 1) * 10;
          var stg = stageName(unlockLv);
          var lockEl = $("sk" + si + "Lock");
          if (lockEl) {
            var lvTxt = isAr ? (" (مستوى " + unlockLv + ")") : (" (Lv " + unlockLv + ")");
            if (to >= unlockLv) {
              lockEl.textContent = "✓ " + stg + lvTxt;
              lockEl.style.background = "rgba(70,224,160,.15)"; lockEl.style.color = "var(--green)";
            } else {
              lockEl.textContent = (isAr ? "🔒 يحتاج: " : "🔒 Needs: ") + stg + lvTxt;
              lockEl.style.background = "var(--surface3)"; lockEl.style.color = "var(--muted)";
            }
          }
    ...
    function compute(){
      if(mode==="weapon"){
        ...
        $("calcResults").innerHTML="<h4>"+L.results+"</h4>"+
          ...
          (myth?statRow("⚙️",lang==="ar"?"عتاد ذهبي":lang==="zh"?"神话齿轮":lang==="ko"?"신화 기어":"Mythic Gears",fmtFull(myth)):"")+
  ...
  /* === حاسبة العتاد — مثريل / تطوير / إتقان === */
  function calcEquip() {
    var L = t(), inp = $("calcInputs");
    var isAr = lang === "ar";
    var G = KS5;
    ...
    function compute(){
      ...
      var gearN = isAr?"عتاد ذهبي":lang==="zh"?"神话齿轮":lang==="ko"?"신화 기어":lang==="es"?"Engranajes míticos":"Mythic Gears";
      if (mode==="enhancement") {
